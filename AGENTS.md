...
- Release tags must use the matching `vX.Y.Z` lightweight tag. Create it with `git tag vX.Y.Z`; do not use `git tag -a`, `git tag -s`, `git tag -m`, or `cog bump --annotated`.
